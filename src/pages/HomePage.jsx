...
function Hero() {
  ...
  return (
    <section className="relative h-screen min-h-[700px] flex items-center justify-center overflow-hidden">
      ...
      <div className="absolute inset-0 bg-gradient-to-t from-cream via-cream/10 to-transparent z-[3] opacity-80" />
      {/* Decorative vignette */}
      ...
      <div className="relative z-[4] text-center px-6 max-w-[900px] mx-auto">
        <div className="flex items-center justify-center gap-5 mb-8 opacity-0" style={{ animation: "hero-fade-in 1s ease 0.3s forwards" }}>
          ...
          <span className="font-label text-[10px] tracking-[0.5em] text-caramel/90 uppercase">Established 2026 · Artisanal Coffee</span>
          <span className="w-12 h-px bg-gradient-to-l from-transparent to-caramel/60" />
        </div>

        <h1
          className="font-display text-[clamp(64px,12vw,140px)] font-bold text-white leading-[0.85] mb-10 tracking-[-0.03em] opacity-0"
...
function BrandMarquee() {
  ...
  return (
    <div className="overflow-hidden bg-ink py-5 border-y border-white/[0.04]">
...
function Philosophy() {
  ...
  return (
    <section className="bg-cream py-24 md:py-36 px-5 md:px-12">
      <div className="max-w-[1360px] mx-auto">
        <Reveal>
          <div className="label text-caramel">Our Philosophy</div>
          <h2 className="font-display text-[clamp(36px,5vw,56px)] font-bold text-brown leading-[1.05] mb-20 tracking-[-0.02em]">
            What We<br /><span className="text-caramel">Stand For</span>
          </h2>
        </Reveal>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-px bg-steam/15">
          {features.map((f, i) => (
            <Reveal key={i} delay={i + 1}>
              <div className="bg-cream group p-10 md:p-14 relative overflow-hidden transition-all duration-500 hover:bg-champagne/60">
                {/* Hover accent line */}
                <div className="absolute top-0 left-0 right-0 h-[2px] bg-caramel scale-x-0 group-hover:scale-x-100 transition-transform duration-500 origin-left" />
                {/* Large decorative number */}
                <span className="font-display text-[80px] md:text-[100px] leading-none font-bold text-brown/[0.04] absolute -top-2 -right-1 select-none pointer-events-none">
                  {String(i + 1).padStart(2, "0")}
                </span>
                <span className="material-symbols-outlined text-[32px] text-caramel mb-8 block">{f.icon}</span>
                <h3 className="font-display text-[22px] font-semibold text-brown mb-4 tracking-[-0.01em]">{f.title}</h3>
                <p className="font-body text-[14px] text-steam leading-[1.8] tracking-[0.005em]">{f.body}</p>
              </div>
            </Reveal>
          ))}
        </div>
...
function OurStory() {
  return (
    <section className="py-28 md:py-44 px-5 md:px-12 max-w-[1360px] mx-auto relative overflow-hidden">
      {/* Decorative background element */}
      <div className="absolute top-20 right-0 w-[400px] h-[400px] bg-caramel/[0.03] rounded-full blur-[100px] pointer-events-none" />

      <div className="grid grid-cols-1 md:grid-cols-2 gap-14 md:gap-28 items-center">
        <Reveal>
          <div className="relative aspect-[4/5] overflow-hidden">
            <ProgressiveImage src="https://images.unsplash.com/photo-1442512595331-e89e73853f31?w=800&q=80" alt="Barista crafting pour-over" />
        ...
        <Reveal delay={2}>
          <div>
            ...
            <h2 className="font-display text-[clamp(38px,5vw,64px)] font-bold text-brown leading-[1.05] mb-8 tracking-[-0.02em]">
              Brewed with Intention,
              <br />
              <span className="text-caramel">Served with Soul</span>
            </h2>
            <p className="font-body text-[16px] text-steam/90 leading-[1.85] max-w-[520px] mb-6 tracking-[0.005em]">
            ...
            <p className="font-body text-[16px] text-steam/90 leading-[1.85] mb-12">
...
function PhotoGallery() {
  ...
  return (
    <>
      <section className="bg-ink/95 py-28 md:py-36">
        <div className="max-w-[1360px] mx-auto px-5 md:px-12 mb-12">
          <Reveal>
            <div className="flex items-end justify-between flex-wrap gap-4">
              <div>
                <div className="label text-caramel">Gallery</div>
                <h2 className="font-display text-[clamp(36px,5vw,56px)] font-bold text-white leading-[1.05] tracking-[-0.02em]">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-1 max-w-[1360px] mx-auto">
          {galleryImages.map((img, i) => (
            <Reveal key={i} delay={i + 1}>
              ...
              >
                ...
                />
                <div className="absolute inset-0 bg-gradient-to-t from-ink/80 via-ink/20 to-transparent opacity-0 group-hover:opacity-100 transition-all duration-400 flex items-end justify-between p-6">
                  <span className="font-display text-[18px] text-white/90 italic">{img.label}</span>
...
function MenuPreview() {
  ...
  return (
    <section className="relative bg-espresso py-28 md:py-40 overflow-hidden">
      ...
      <div className="relative max-w-[1360px] mx-auto px-5 md:px-12">
        <Reveal>
          ...
          <div className="flex justify-between items-end flex-wrap gap-4 mb-0">
            <h2 className="font-display text-[clamp(38px,5vw,64px)] font-bold text-white leading-[1.05] tracking-[-0.02em]">
              Handcrafted
              <br />
        ...
        <div className="flex gap-1 flex-wrap my-12">
          {tabs.map((t) => (
            <button
              key={t.id}
              onClick={() => setActiveTab(t.id)}
              className={`px-7 py-3 border-none cursor-pointer font-body text-[11px] font-semibold tracking-[0.14em] uppercase transition-all duration-300 rounded-full ${
                activeTab === t.id
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-[repeat(auto-fill,minmax(280px,1fr))] gap-4 md:gap-5">
          {menuData[activeTab]?.map((item, i) => (
            <Reveal key={i} delay={Math.min(i + 1, 4)}>
              <div className="glass-card group rounded-2xl overflow-hidden cursor-pointer bg-white/[0.03] border-white/[0.07] hover:bg-white/[0.07]">
                <div className="relative overflow-hidden">
                  <img src={item.img} alt={item.name} className="w-full aspect-[4/3] object-cover transition-transform duration-600 group-hover:scale-[1.06]" />
                ...
                <div className="p-6 pb-5">
                  <h3 className="font-display text-xl font-semibold text-white mb-2 leading-tight tracking-[-0.01em]">{item.name}</h3>
...
function SeasonalBanner() {
  ...
  return (
    <section className="relative py-28 md:py-44 px-5 md:px-12 text-center overflow-hidden">
      ...
      <div className="absolute inset-0 -z-10 bg-[radial-gradient(ellipse_at_center,rgba(56,68,22,0.35)_0%,rgba(26,21,16,0.95)_100%)]" />

      <Reveal>
        <div className="relative z-10 max-w-[700px] mx-auto">
          <div className="label text-[#7fa640]">{seasonal.badge || "Seasonal Special"}</div>
          <h2 className="font-display text-[clamp(36px,6vw,64px)] font-bold text-white mb-4 leading-[1.05] tracking-[-0.02em]">
            {seasonal.title}
          </h2>
          <div className="font-display text-2xl text-[#a8cb6a] mb-10">{seasonal.price} · Available Now</div>
          <p className="text-white/40 text-[16px] max-w-[480px] mx-auto mb-12 leading-[1.8] tracking-[0.005em]">{seasonal.desc}</p>

          <div className="flex gap-3 sm:gap-5 justify-center mb-12">
            {[
              { v: timeLeft.d, l: "Days" },
              { v: timeLeft.h, l: "Hrs" },
              { v: timeLeft.m, l: "Min" },
              ...
              <div key={l} className="glass rounded-2xl px-5 py-4 sm:px-7 sm:py-5 text-center min-w-[60px] sm:min-w-[76px]" style={{ animation: "glow-pulse 3s ease-in-out infinite" }}>
                <div className="font-display text-2xl sm:text-4xl font-bold text-white leading-none tabular-nums">{String(v).padStart(2, "0")}</div>
                <div className="font-label text-[8px] sm:text-[9px] tracking-[0.3em] text-white/35 uppercase mt-2">{l}</div>
              </div>
            ))}
          </div>
  ...
  return (
    <section className="bg-parchment/40">
      <div className="grid grid-cols-1 md:grid-cols-[1fr_1.5fr] min-h-[600px]">
        <Reveal>
          <div className="bg-parchment/60 backdrop-blur-sm p-10 md:p-16 md:pr-20 flex flex-col justify-center">
            <div className="label text-caramel">Find Us</div>
            <h2 className="font-display text-[clamp(38px,5vw,64px)] font-bold text-brown leading-[1.05] mb-10 tracking-[-0.02em]">
              Visit Our
              <br />
              <span className="text-caramel">Space</span>
            </h2>

            <div className="bg-white/50 backdrop-blur-sm rounded-2xl p-5 mb-4 flex gap-4 border-l-[3px] border-l-caramel shadow-sm">
              <span className="material-symbols-outlined text-2xl text-caramel mt-0.5 shrink-0">location_on</span>
              <div>
                <div className="font-display text-lg font-semibold text-brown">{loc.name}</div>
                <div className="font-body text-[14px] text-steam/80 mt-1 leading-relaxed">{loc.address}</div>
                <div className="font-body text-[12px] text-caramel/80 mt-1 tracking-wide leading-relaxed">{loc.hours}</div>
              </div>
            </div>

            <div className="bg-white/50 backdrop-blur-sm rounded-2xl p-5 mb-10 flex gap-4 shadow-sm">
              <span className="material-symbols-outlined text-2xl text-caramel mt-0.5 shrink-0">wifi</span>
              <div>
                <div className="font-label text-[9px] tracking-[0.3em] text-caramel/80 uppercase mb-1">Amenities</div>
                <div className="font-body text-[14px] text-brown/80 leading-relaxed">Free Wi-Fi · Outdoor Seating · Takeaway</div>
              </div>
            </div>
...
function Testimonials() {
  ...
  return (
    <section className="relative bg-cream py-28 md:py-40 px-5 md:px-12 text-center overflow-hidden">
      {/* Subtle warm radial glow */}
      <div className="absolute inset-0 bg-[radial-gradient(circle_at_50%_30%,rgba(192,125,58,0.06)_0%,transparent_65%)]" />
      ...
      <div className="relative max-w-[800px] mx-auto">
        ...
        <p className={`font-display text-[clamp(22px,3.5vw,34px)] italic text-brown leading-snug mb-8 transition-all duration-500 ${animating ? "opacity-0 -translate-y-3" : "opacity-100 translate-y-0"}`}>
          {testimonials[idx].text}
        </p>
        <span className={`inline-block font-body text-[12px] tracking-[0.22em] text-steam/70 uppercase transition-all duration-500 ${animating ? "opacity-0 translate-y-2" : "opacity-100 translate-y-0"}`}>
        ...
        <div className="flex gap-3 justify-center mt-14">
          {testimonials.map((_, i) => (
            <button
              ...
              className={`w-2.5 h-2.5 rounded-full border-none cursor-pointer transition-all duration-300 ${idx === i ? "bg-caramel scale-125" : "bg-parchment hover:bg-caramel/40"}`}
